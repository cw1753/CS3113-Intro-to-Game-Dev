...
unsigned char levelData[LEVEL_HEIGHT][LEVEL_WIDTH] =
{
	...
	{ 2, 2, 2, 2, 2, 2, 3, 0, 0, 0, 0, 0, 0, 0, 0, 3, 2, 2, 2, 2, 2, 2 },
	{ 32, 33, 33, 34, 32, 33, 33, 101, 100, 101, 100, 101, 100, 101, 100, 33, 34, 32, 33, 32, 33, 33 }
};
...
void updateGame(float time){
	...
	else if (myPlayer.y - myPlayer.height / 2 < (-TILE_SIZE * LEVEL_HEIGHT ) && falling){
		Mix_PlayChannel(2, loseSound, 0);
		falling = 0;
	}
	else if (myPlayer.y - myPlayer.height / 2 < (-TILE_SIZE * LEVEL_HEIGHT - (60 * TILE_SIZE))){
		state = State_Game_Over;
	}
}
void updateGameOver(){}
...
void setup(){
	...
	collectSound = Mix_LoadWAV("King.wav");
	winSound = Mix_LoadWAV("Winning.wav");
	loseSound = Mix_LoadWAV("falling.wav");
	bkMusic = Mix_LoadMUS("Night_Owl.mp3");
	if (Mix_PlayMusic(bkMusic, 1)){
		test = 1;
	}
